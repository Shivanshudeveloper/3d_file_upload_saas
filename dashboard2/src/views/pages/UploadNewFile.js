...
const UploadNewFile = () => {
    ...
    const [deliveryDate, setDeliveryDate] = React.useState(new Date().toLocaleDateString());
    ...
    function createData(name, cut, contentType, uploadedTo, status, desiredReception, date) {
       return {name, cut, contentType, uploadedTo, status, desiredReception, date};
    }

    const firebaseUpload = () => {
      acceptedFiles.forEach(file => {
        storage.ref(`/files/${file.path}`).put(file).on("state_changed", (snapshot) => {
    ...
    const fileSubmit = () => {
        ...
        acceptedFiles.forEach(file => {
            updatedRows.push(createData(file.path, null, file.type, new Date().toLocaleDateString(), 'Pending', comment, deliveryDate));
        })
